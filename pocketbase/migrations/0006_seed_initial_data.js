migrate(
  (app) => {
    const users = app.findCollectionByNameOrId('_pb_users_auth_')
    let user
    try {
      user = app.findAuthRecordByEmail('_pb_users_auth_', 'brunavia@gmail.com')
    } catch (e) {
      user = new Record(users)
      user.setEmail('brunavia@gmail.com')
      user.setPassword('securepassword123')
      user.setVerified(true)
      user.set('name', 'Administrador')
      user.set('password_hash', 'dummy_hash_for_seed')
      app.save(user)
    }

    const dateNow = new Date().toISOString().split('T')[0] + ' 10:00:00.000Z'

    // Seed Lançamento
    const lancamentos = app.findCollectionByNameOrId('lancamentos')
    const l1 = new Record(lancamentos)
    l1.set('user_id', user.id)
    l1.set('data', dateNow)
    l1.set('descricao', 'Venda Consultoria (Cliente A)')
    l1.set('categoria', 'Serviços')
    l1.set('valor', 5000)
    l1.set('tipo', 'entrada')
    app.save(l1)

    // Seed Contas a Pagar
    const contas_pagar = app.findCollectionByNameOrId('contas_pagar')
    const cp1 = new Record(contas_pagar)
    cp1.set('user_id', user.id)
    cp1.set('data', dateNow)
    cp1.set('favorecido', 'Google Cloud')
    cp1.set('categoria', 'Infraestrutura')
    cp1.set('descricao', 'Servidores')
    cp1.set('valor', 850)
    cp1.set('status', 'previsto')
    app.save(cp1)

    // Seed Atividades
    const atividades = app.findCollectionByNameOrId('atividades')
    const a1 = new Record(atividades)
    a1.set('user_id', user.id)
    a1.set('data', dateNow)
    a1.set('descricao', 'Revisar fluxo de caixa do mês')
    a1.set('status', 'Aguardando')
    a1.set('responsavel', 'Financeiro')
    app.save(a1)
  },
  (app) => {
    // Cascaded or wiped by previous migrations
  },
)
